#!/usr/bin/env python
"""Regenerate the shared cross-language test fixture (points.json).
...
def main():
    ...
    for p in points:
        ...
        p["land_fraction"] = (None if r.land_fraction is None
                              else round(r.land_fraction, 6))
        p["cell"] = r.cell
    ...
    # refined fixture: deterministic points inside coastal cells, answers
    # with the OSM refinement loaded (skipped if the TFLR is not built)
    tflr = Path(__file__).resolve().parent.parent / "data" / "coastal_osm_L10.tflr"
    if not tflr.exists():
        print(f"no {tflr.name}; skipping refined fixture")
        return
    from _fastloc import index_to_lonlat_ring
    lc = LandCheck(refine_path=tflr)
    coastal = [i for s, e, c in zip(lc._starts, lc._ends, lc._coastal)
               if c for i in range(s, e)]
    rng = random.Random(424242)
    refined_points = []
    for _ in range(500):
        idx = coastal[rng.randrange(len(coastal))]
        ...
        refined_points.append({
            ...
            "land": r.land, "kind": r.kind, "refined": r.refined,
    ...
    print(f"wrote {out2}: {len(refined_points)} coastal points, {n_ref} refined")
