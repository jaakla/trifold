#!/usr/bin/env python3
"""Benchmark landcheck polyline queries.

Generates a set of random global polylines, runs landcheck polyline
classification (both base and coastal-refined, both uniform and vertex modes),
and outputs a JSON report. Follows the same format as benchmark_landcheck.py.
"""
...
sys.path.insert(0, str(REPO / "landcheck" / "python"))
sys.path.insert(0, str(REPO / "src"))

from landcheck import LandCheck  # noqa: E402

TFLS = REPO / "landcheck" / "data" / "landsea_L10.tfls"
TFLR = REPO / "landcheck" / "data" / "coastal_osm_L10.tflr"
...
def classify_all(
    checker: LandCheck, polylines: list[list[tuple[float, float]]],
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=Path("/private/tmp/trifold-landcheck-polyline-bench"))
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    report_path = args.output_dir / "landcheck_polyline_benchmark.json"

    polylines = generate_polylines(args.n_polylines, seed=20_260_615)

    # compute total km across all polylines
    ...
    # --- refined ---
    refined, refined_load = load_timed(refine_path=TFLR)
    ...
    report = {
        ...
        "landcheck_base": {
            ...
            "dataset_bytes": TFLS.stat().st_size,
            "uniform": {
        ...
        "landcheck_refined": {
            ...
            "dataset_bytes": TFLR.stat().st_size,
            "base_dataset_bytes": TFLS.stat().st_size,
            "uniform": {
